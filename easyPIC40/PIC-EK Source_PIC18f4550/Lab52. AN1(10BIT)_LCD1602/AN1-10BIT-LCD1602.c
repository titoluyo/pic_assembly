...
main(void)
{
	...
	ADFM=1;                 //转换后数据右移，则ADRESL内数据为低8位数据
    ADIE=0;		            //禁止AD中断  
	lcdreset();             //初始化1602
	lcd_write_char(2,0,'-');   	
	lcd_write_char(3,0,'-');   		
	...
	lcd_write_char(13,0,'-');
  	...
  	{    	 	
    	...
    	disbuf=ADRESH*256+ADRESL; 
		lcd_write_char(5,1,'A');   	
		lcd_write_char(6,1,'D');   		
		lcd_write_char(7,1,'=');  
		lcd_write_char(8,1,(disbuf/1000)+0x30);   	//千位
    	lcd_write_char(9,1,((disbuf%1000)/100)+0x30);   	//百位
		lcd_write_char(10,1,(disbuf%100)/10+0x30);   //十位
		lcd_write_char(11,1,(disbuf%10)+0x30);       //个位	
  	}
}
