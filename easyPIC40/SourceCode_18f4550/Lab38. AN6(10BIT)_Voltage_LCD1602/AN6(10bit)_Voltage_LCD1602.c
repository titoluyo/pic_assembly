...
main(void)
{
	...
	lcd_write_char(13,0,'-');
  	...
  	{    	
		...
		disbuf=disbuf/16;		
		i_val=(double)disbuf;
		i_val=(i_val*50)/102.3;
		f_val = modf(i_val, &i_val);
		if(i_val>999) i_val=999;
		disbuf=(int)i_val;

		lcd_write_char(1,1,'V');   	
		lcd_write_char(2,1,'o');   		
		lcd_write_char(3,1,'l');  
		lcd_write_char(4,1,'t');   	
		lcd_write_char(5,1,'a');   		
		lcd_write_char(6,1,'g');
		lcd_write_char(7,1,'e');   	
		lcd_write_char(8,1,':');   		

    	lcd_write_char(10,1,(disbuf/100)+0x30);   	//百位
		lcd_write_char(11,1,'.');
		lcd_write_char(12,1,(disbuf%100)/10+0x30);   //十位
		lcd_write_char(13,1,(disbuf%10)+0x30);       //个位	

		lcd_write_char(14,1,'V');
  	}
}
